...
Some of the helper methods required from the UserDetails interface (e.g. checking credential expiry, checking account expiry) is only needed during the authentication process.
Similarly, some methods available to the Authentication interface are only used once the authentication is successful - not during.
...
If we require custom authentication requirement that is not fulfilled by Spring Security, we can build our own authentication logic by implementing the **AuthenticationProvider interface**.
Based on the type of the authentication object, our **ProviderManager** can check with all the implementations of the AuthenticationProvider to authenticate the user.

![img.png](spring-security-authentication-provider.png)

* The **authenticate()** method is where we define our authentication logic. The Provider Manager will invoke this method with the authentication object, which contains the user details. 
An Authentication object must be returned to indicate if the authentication is successful or not. 
* The **supports()** method is for us to tell the framework what types of authentication our Authentication Provider should support.

---

### Security Context

The SecurityContextHolder contains methods to manage the **SecurityContext** (e.g. getContext, clearContext). 
The SecurityContext is where Spring Security stores the details of who is authenticated. 
If it contains a value, it is used as the currently authenticated user.

![img.png](spring-security-context-holder.png)

During the authentication process, an Authentication object is created. This object contains details such as:
* **Principal**: the name or username of the authenticated user.
* **Credentials**: the password of the user.
* **Authorities**: the authorities of the user.

When authentication is completed, the authentication details are stored inside a SecurityContext object.
It uses a **ThreadLocal** (MODE_THREADLOCAL) object to store the security context, ensuring it is always available to methods in the same thread.
Other holding strategies for the security context: MODE_INHERITABLETHREADLOCAL and MODE_GLOBAL. 

We can load the login user details by:
1. Load the currently authenticated user details via a **static call** to the SecurityContextHolder.
2. Mention the Authentication object directly as a **method argument**, and the framework will correctly resolve it by extracting from the SecurityContextHolder.
